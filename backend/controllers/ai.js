...
4. STRICT TABLE EXTRACTION: If tabular data exists, convert it strictly to clean HTML <table> tags. Follow these rules EXACTLY:
   ...
   i) NESTED / INNER COLUMNS (FINANCIAL STATEMENTS): In financial statements (like Balance Sheets), you often see a main "Liabilities" column, an inner "₹" column (for sub-totals), another main "Assets" column, and its inner "₹" column. Sometimes amounts are listed in the inner column and the final total in the outer column. You MUST output EXACTLY the maximum number of columns visible across the widest part of the table. If a row only has an item name and an outer amount, you MUST insert an empty <td></td> for the missing inner amount column so the outer amount aligns correctly under the outer header. The number of columns MUST be exactly the same as in the image and the numbers MUST be in the exact right column as they appear in the image. NEVER shift data leftward into the wrong column.
