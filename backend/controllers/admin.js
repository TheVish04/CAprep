...
// GET /api/admin/contact/general-queries - List general inquiries from Contact Us (admin only)
router.get('/contact/general-queries', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit, 10) || 50, 200);
    const submissions = await ContactSubmission.find({ type: 'contact' })
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();
    res.status(200).json({ success: true, data: submissions });
  } catch (error) {
    sendErrorResponse(res, 500, { message: 'Failed to fetch general queries', error });
  }
});
