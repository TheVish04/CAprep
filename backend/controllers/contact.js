...
const { featureRequestSchema, issueReportSchema, generalContactSchema } = require('../validators/contactValidator');

// POST /api/contact/general - Submit general inquiry (open to public)
router.post('/general', async (req, res) => {
  try {
    const { error, value } = generalContactSchema.validate(req.body || {}, { abortEarly: false });
    if (error) {
    ...
    // We expect name and email from the body since it's open to the public
    ...
    res.status(201).json({ success: true, id: doc._id, message: 'Message submitted successfully' });
  } catch (err) {
    sendErrorResponse(res, 500, { message: 'Failed to submit message', error: err });
  }
});
